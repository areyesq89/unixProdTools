...
Change directory to `Newdirectory` directory, create a new directory called `FirstExercises` and move all contents of `Newdirectory` into `FirstExercises`.
...
The example below, stores the output of the `cut` line into a variable `i`. Then uses a for loop to print each of the elements stored in `i`.
...
for x in `echo ${i}`
  do 
    echo ${x} 
	cut -f1 annotation.gtf | grep -x ${x} | wc -l
  done
```
