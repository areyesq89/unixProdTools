...
AKA: the thing that became famous 20 years ago with the movie "The Matrix".

![](https://wsvincent.com/assets/images/terminal-command-line-for-beginners/matrix.gif)

## Terminal (real life)

It is a text-only interface to interact with a computer. It receives standard input from a user and results in standard output and standard error. 
...
Change directory to `Newdirectory` directory, create a new directory called `FirstExercises` and move all contents into `FirstExercises`.
...
There are several commands to decompress files (`gunzip`, `unzip`, `bzip2`). They are used depending on the compression algorithm that is typically shown in the extension of the file. Our annotation file has the termination *gz* and we will use the corresponding command (`gunzip`) to decompress our file:
...
How many times does chromosome "V" appears? Note that if we use the pattern "V" for `grep`, we will also get other chromosomes that match, including "V", "VI", "IV", and so on. To identify **only** the "V" pattern, we will need to use `^` to specify that the line starts with "V" and `$` to specify that the line finishes "V":
...
The example below, store the output of the `cut` line into a variable `i`. Then will use a for loop to print each of the elements stored in `i`.
...
Count the number of times each chromosome appears in the file. Hint: combine the code from the two previous slides. 
